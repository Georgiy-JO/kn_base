...
* System settings (don't forget buttons);
  * Go to region settings and add languages (***mozc*** will be installed)
...
- Hiddify
  1.  
    ...
    ```
  2. Download AppImage - [link](https://github.com/hiddify/hiddify-app/releases)
...
- Other (choose the ones you need):
    ```bash
    sudo apt install python3-pip unzip unrar cargo p7zip ntfs-3g neofetch mpv vlc check gcovr lcov openssh-client openssh-server mc htop sl sshpass valgrind cppcheck clang-format gcc g++ make cmake git gthumb rhythmbox ipcalc postgresql curl git-lfs -y
    ```

### 16. Add more repositories
#### Flat pack
```bash
